...
public final class BadgeState {
    ...
    BadgeState(Context context, int badgeResId, int defStyleAttr, int defStyleRes, State storedState) {
        ...
        state22.autoAdjustToWithinGrandparentBounds = Boolean.valueOf(zBooleanValue);
        a.recycle();
        Locale locale = storedState.numberLocale;
        State state23 = this.currentState;
        if (locale == null) {
            state23.numberLocale = Locale.getDefault(Locale.Category.FORMAT);
        } else {
            state23.numberLocale = storedState.numberLocale;
        }
