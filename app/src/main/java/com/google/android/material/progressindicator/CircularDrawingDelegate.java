...
final class CircularDrawingDelegate extends DrawingDelegate<CircularProgressIndicatorSpec> {
    ...
    void fillIndicator(Canvas canvas, Paint paint, float startFraction, float endFraction, int color) {
        ...
        float startDegree = this.arcDirectionFactor * startFraction * 360.0f;
        int i = this.arcDirectionFactor;
        float arcDegree = endFraction >= startFraction ? (endFraction - startFraction) * 360.0f * i : ((1.0f + endFraction) - startFraction) * 360.0f * i;
        ...
        float arcDegree2 = arcDegree;
        if (this.displayedCornerRadius > 0.0f && Math.abs(arcDegree2) < 360.0f) {
            paint.setStyle(Paint.Style.FILL);
            drawRoundedEnd(canvas, paint, this.displayedTrackThickness, this.displayedCornerRadius, startDegree);
            drawRoundedEnd(canvas, paint, this.displayedTrackThickness, this.displayedCornerRadius, startDegree + arcDegree2);
        }
    }
