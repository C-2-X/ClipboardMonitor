...
class UtcDates {
    ...
    static String getDatePatternAsInputFormat(String localeFormat) {
        return localeFormat.replaceAll("[^dMy/\\-.]", HttpUrl.FRAGMENT_ENCODE_SET).replaceAll("d{1,2}", "dd").replaceAll("M{1,2}", "MM").replaceAll("y{1,4}", "yyyy").replaceAll("\\.$", HttpUrl.FRAGMENT_ENCODE_SET).replaceAll("My", "M/y");
    }
