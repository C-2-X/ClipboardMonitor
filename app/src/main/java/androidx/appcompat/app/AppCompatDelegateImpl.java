...
class AppCompatDelegateImpl extends AppCompatDelegate implements MenuBuilder.Callback, LayoutInflater.Factory2 {
    ...
    public Context attachBaseContext2(Context baseContext) {
        ...
        if (!sCanApplyOverrideConfiguration || !(baseContext instanceof ContextThemeWrapper)) {
            ...
            try {
                ContextThemeWrapperCompatApi17Impl.applyOverrideConfiguration((ContextThemeWrapper) baseContext2, config);
                return baseContext2;
        ...
        Configuration referenceConfig = Api17Impl.createConfigurationContext(baseContext2, overrideConfig).getResources().getConfiguration();
        Configuration baseConfig2 = baseContext2.getResources().getConfiguration();
        referenceConfig.uiMode = baseConfig2.uiMode;
        if (!referenceConfig.equals(baseConfig2)) {
            Configuration configOverlay = generateConfigDelta(referenceConfig, baseConfig2);
            baseConfig = configOverlay;
        } else {
            baseConfig = null;
        }
        Configuration config3 = createOverrideAppConfiguration(baseContext2, modeToApply, localesToApply, baseConfig, true);
    ...
    */
    ...
    private int getActivityHandlesConfigChangesFlags(Context baseContext) {
        if (!this.mActivityHandlesConfigFlagsChecked && (this.mHost instanceof Activity)) {
            ...
            try {
                ActivityInfo info = pm.getActivityInfo(new ComponentName(baseContext, this.mHost.getClass()), 269221888);
                if (info != null) {
