...
public class DiffUtil {
    ...
    public static DiffResult calculateDiff(Callback cb, boolean detectMoves) {
        ...
        while (!stack.isEmpty()) {
            ...
            int[] backward2 = backward;
            int[] forward2 = forward;
            Snake snake = diffPartial(cb, range.oldListStart, range.oldListEnd, range.newListStart, range.newListEnd, forward2, backward2, max);
            forward = forward2;
            if (snake != null) {
                if (snake.size > 0) {
                    snakes.add(snake);
    ...
    private static Snake diffPartial(Callback cb, int startOld, int endOld, int startNew, int endNew, int[] forward, int[] backward, int kOffset) {
        ...
        while (d <= dLimit) {
            int k = -d;
            while (k <= d) {
                if (k == (-d) || (k != d && forward[(kOffset + k) - i2] < forward[kOffset + k + i2])) {
                    int x3 = kOffset + k;
                    x2 = forward[x3 + i2];
                    removal2 = false;
                } else {
    ...
    public static class DiffResult {
        ...
        public void dispatchUpdatesTo(ListUpdateCallback updateCallback) {
            ...
            for (int snakeIndex = diffResult.mSnakes.size() - 1; snakeIndex >= 0; snakeIndex--) {
                Snake snake = diffResult.mSnakes.get(snakeIndex);
                int snakeSize = snake.size;
                int endX = snake.x + snakeSize;
                int endY = snake.y + snakeSize;
                if (endX < posOld2) {
                    diffResult.dispatchRemovals(postponedUpdates, batchingCallback, endX, posOld2 - endX, endX);
                }
                if (endY >= posNew2) {
                    diffResult = this;
                } else {
                    diffResult = this;
                    diffResult.dispatchAdditions(postponedUpdates, batchingCallback, endX, posNew2 - endY, endY);
                }
                int i = snakeSize - 1;
                while (i >= 0) {
                    if ((diffResult.mOldItemStatuses[snake.x + i] & 31) != 2) {
                        z = z2;
                    } else {
                        z = true;
