...
public class AnimatorInflaterCompat {
    ...
    private static void parseAnimatorFromTypeArray(ValueAnimator anim, TypedArray arrayAnimator, TypedArray arrayObjectAnimator, float pixelSize, XmlPullParser parser) {
        long duration = TypedArrayUtils.getNamedInt(arrayAnimator, parser, TypedValues.TransitionType.S_DURATION, 1, MaterialCardViewHelper.DEFAULT_FADE_ANIM_DURATION);
        ...
        if (TypedArrayUtils.hasAttribute(parser, "valueFrom") && TypedArrayUtils.hasAttribute(parser, "valueTo")) {
            ...
            PropertyValuesHolder pvh = getPVH(arrayAnimator, valueType, 5, 6, HttpUrl.FRAGMENT_ENCODE_SET);
            if (pvh != null) {
                anim.setValues(pvh);
            }
        }
