    ...
    public long solverMeasure(ConstraintWidgetContainer layout, int optimizationLevel, int paddingX, int paddingY, int widthMode, int widthSize, int heightMode, int heightSize, int lastMeasureWidth, int lastMeasureHeight) {
        ...
        if (!optimize2) {
            ...
            while (i2 < childCount2) {
                ...
                boolean ratio = matchWidth2 && matchHeight2 && child.getDimensionRatio() > 0.0f;
        ...
        boolean optimize4 = ((widthMode == 1073741824 && heightMode == 1073741824) || optimizeWrap2) & matchHeight;
        if (!optimize4) {
            allSolved = false;
        ...
        if (sizeDependentWidgetsCount <= 0) {
            ...
            int minWidth = Math.max(constraintWidgetContainer.getWidth(), basicMeasure.constraintWidgetContainer.getMinWidth());
            int minHeight = Math.max(constraintWidgetContainer.getHeight(), basicMeasure.constraintWidgetContainer.getMinHeight());
            ...
            while (startingWidth3 < sizeDependentWidgetsCount) {
                ...
                if (!(widget instanceof VirtualLayout)) {
                    ...
                    needSolverPass = needSolverPass3 | virtualLayout.needSolverPass();
                }
                startingWidth3 = i3 + 1;
            ...
            while (j2 < maxIterations2) {
                ...
                boolean needSolverPass5 = needSolverPass4;
                ...
                int startingWidth4 = startingWidth2;
                solveLinearSystem(constraintWidgetContainer, "intermediate pass", j2 + 1, startingWidth4, startingHeight3);
                needSolverPass4 = false;
                j2++;
                sizeDependentWidgetsCount = sizeDependentWidgetsCount4;
                startingWidth2 = startingWidth4;
