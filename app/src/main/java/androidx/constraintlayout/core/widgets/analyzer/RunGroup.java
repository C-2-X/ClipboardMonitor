...
class RunGroup {
    ...
    public long computeWrapSize(ConstraintWidgetContainer container, int orientation) {
        ...
        if (runWithStartTarget && runWithEndTarget) {
            ...
            return (((long) this.firstRun.start.margin) + runDimension) - ((long) this.firstRun.end.margin);
        }
        if (runWithStartTarget) {
            long maxPosition2 = traverseStart(this.firstRun.start, this.firstRun.start.margin);
            long runDimension2 = ((long) this.firstRun.start.margin) + dimension;
            return Math.max(maxPosition2, runDimension2);
        }
        WidgetRun widgetRun2 = this.firstRun;
        if (runWithEndTarget) {
            long minPosition2 = traverseEnd(widgetRun2.end, this.firstRun.end.margin);
            long runDimension3 = ((long) (-this.firstRun.end.margin)) + dimension;
            return Math.max(-minPosition2, runDimension3);
        }
        return (((long) widgetRun2.start.margin) + this.firstRun.getWrapDimension()) - ((long) this.firstRun.end.margin);
    }
            ...
            if (dependency2 instanceof DependencyNode) {
                ...
                if (node2.run != run && node2 == node2.run.start) {
                    if (run instanceof ChainRun) {
                        ...
                        for (WidgetRun widgetChainRun2 : chainRun2.widgets) {
