        ...
        switch (actionMasked) {
            ...
            case 2:
                ...
                if (activePointerIndex == -1) {
                    ...
                    int deltaY3 = deltaY2 - releaseVerticalGlow(deltaY2, ev.getX(activePointerIndex));
                    ...
                    if (this.mIsBeingDragged) {
                        ...
                        int deltaY4 = deltaY;
                        boolean clearVelocityTracker = overScrollByCompat(0, deltaY, 0, getScrollY(), 0, range, 0, 0, true) && !hasNestedScrollingParent(0);
                        int scrolledDeltaY = getScrollY() - oldY;
                        int unconsumedY = deltaY4 - scrolledDeltaY;
                        this.mScrollConsumed[1] = 0;
                        dispatchNestedScroll(0, scrolledDeltaY, 0, unconsumedY, this.mScrollOffset, 0, this.mScrollConsumed);
                        this.mLastMotionY -= this.mScrollOffset[1];
                        this.mNestedYOffset += this.mScrollOffset[1];
                        if (canOverscroll) {
                            int deltaY5 = deltaY4 - this.mScrollConsumed[1];
                            int pulledToY = oldY + deltaY5;
                            if (pulledToY < 0) {
                                EdgeEffectCompat.onPullDistance(this.mEdgeGlowTop, (-deltaY5) / getHeight(), ev.getX(activePointerIndex) / getWidth());
                                if (!this.mEdgeGlowBottom.isFinished()) {
                                    this.mEdgeGlowBottom.onRelease();
                                }
                            } else if (pulledToY > range) {
                                EdgeEffectCompat.onPullDistance(this.mEdgeGlowBottom, deltaY5 / getHeight(), 1.0f - (ev.getX(activePointerIndex) / getWidth()));
        ...
        if (event.getAction() == 8 && !this.mIsBeingDragged) {
            ...
            if (vscroll != 0.0f) {
                ...
                if (newScrollY < 0) {
                    ...
                    if (canOverScroll) {
                        EdgeEffectCompat.onPullDistance(this.mEdgeGlowTop, (-newScrollY) / getHeight(), 0.5f);
                        this.mEdgeGlowTop.onRelease();
                        invalidate();
                        absorbed = true;
                    }
                    ...
                    if (canOverScroll) {
                        EdgeEffectCompat.onPullDistance(this.mEdgeGlowBottom, (newScrollY - range) / getHeight(), 0.5f);
                        this.mEdgeGlowBottom.onRelease();
                        invalidate();
                        absorbed = true;
                    }
