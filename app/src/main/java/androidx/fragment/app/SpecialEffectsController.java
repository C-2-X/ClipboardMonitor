    ...
    void forceCompleteAllOperations() {
        ...
        synchronized (this.mPendingOperations) {
            ...
            for (Operation operation : runningOperations) {
                if (FragmentManager.isLoggingEnabled(2)) {
                    Log.v("FragmentManager", "SpecialEffectsController: " + (attachedToWindow ? HttpUrl.FRAGMENT_ENCODE_SET : "Container " + this.mContainer + " is not attached to window. ") + "Cancelling running operation " + operation);
                }
                operation.cancel();
            }
            ...
            for (Operation operation2 : pendingOperations) {
                if (FragmentManager.isLoggingEnabled(2)) {
                    Log.v("FragmentManager", "SpecialEffectsController: " + (attachedToWindow ? HttpUrl.FRAGMENT_ENCODE_SET : "Container " + this.mContainer + " is not attached to window. ") + "Cancelling pending operation " + operation2);
                }
                operation2.cancel();
            }
        }
