...
public final class UProgressionUtilKt {
    ...
    private static final int m1380differenceModuloWZ9TVnA(int a, int b, int c) {
        ...
        return UInt.m265constructorimpl(Integer.compareUnsigned(ac, bc) >= 0 ? ac - bc : UInt.m265constructorimpl(ac - bc) + c);
    }
    ...
    private static final long m1381differenceModulosambcqE(long a, long b, long c) {
        ...
        return ULong.m344constructorimpl(Long.compareUnsigned(ac, bc) >= 0 ? ac - bc : ULong.m344constructorimpl(ac - bc) + c);
    }
    ...
    public static final int m1383getProgressionLastElementNkh28Cs(int start, int end, int step) {
        if (step > 0) {
            if (Integer.compareUnsigned(start, end) < 0) {
                return UInt.m265constructorimpl(end - m1380differenceModuloWZ9TVnA(end, start, UInt.m265constructorimpl(step)));
            }
        } else {
            if (step >= 0) {
            ...
            if (Integer.compareUnsigned(start, end) > 0) {
                return UInt.m265constructorimpl(m1380differenceModuloWZ9TVnA(start, end, UInt.m265constructorimpl(-step)) + end);
            }
        }
        return end;
    }
    ...
    public static final long m1382getProgressionLastElement7ftBX0g(long start, long end, long step) {
        if (step > 0) {
            if (Long.compareUnsigned(start, end) < 0) {
                return ULong.m344constructorimpl(end - m1381differenceModulosambcqE(end, start, ULong.m344constructorimpl(step)));
            }
        } else if (step < 0) {
            if (Long.compareUnsigned(start, end) > 0) {
                return ULong.m344constructorimpl(m1381differenceModulosambcqE(start, end, ULong.m344constructorimpl(-step)) + end);
            }
        } else {
            throw new IllegalArgumentException("Step is zero.");
        }
