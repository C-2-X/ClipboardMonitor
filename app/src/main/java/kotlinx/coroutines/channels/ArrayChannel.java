...
public class ArrayChannel<E> extends AbstractChannel<E> {
    ...
    protected Object offerInternal(E element) {
        ...
        try {
            ...
            Symbol symbolUpdateBufferSize = updateBufferSize(size);
        ...
        try {
            ...
            if (size == 0) {
                do {
                    ...
                    failure = select.performAtomicTrySelect(tryOfferDescDescribeTryOffer);
                    if (failure == null) {
                        this.size = size;
                        Object receive = tryOfferDescDescribeTryOffer.getResult();
                        Unit unit = Unit.INSTANCE;
                        reentrantLock.unlock();
        ...
        try {
            ...
            if (size == this.capacity) {
                while (true) {
                    ...
                    if (failure == null) {
                        send = tryPollDescDescribeTryPoll.getResult();
                        success = true;
                        Intrinsics.checkNotNull(send);
                        replacement = ((Send) send).getElement();
                        break;
                    }
    ...
    protected void onCancelIdempotent(boolean wasClosed) {
        ...
        try {
            ...
            for (int i2 = 0; i2 < i; i2++) {
                ...
                if (function1 != null && value != AbstractChannelKt.EMPTY) {
                    undeliveredElementExceptionCallUndeliveredElementCatchingException = OnUndeliveredElementKt.callUndeliveredElementCatchingException(function1, value, undeliveredElementExceptionCallUndeliveredElementCatchingException);
                }
