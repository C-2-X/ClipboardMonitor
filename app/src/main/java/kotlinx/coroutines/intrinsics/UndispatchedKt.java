...
public final class UndispatchedKt {
    public static final <T> void startCoroutineUnintercepted(Function1<? super Continuation<? super T>, ? extends Object> function1, Continuation<? super T> continuation) {
        ...
        try {
            ...
            actualCompletion$iv.resumeWith(Result.m170constructorimpl(value$iv));
        } catch (Throwable e$iv) {
            Result.Companion companion2 = Result.INSTANCE;
            actualCompletion$iv.resumeWith(Result.m170constructorimpl(ResultKt.createFailure(e$iv)));
        }
    }
    ...
    private static final <T> void startDirect(Continuation<? super T> continuation, Function1<? super Continuation<? super T>, ? extends Object> function1) {
        ...
        try {
            ...
            if (value != IntrinsicsKt.getCOROUTINE_SUSPENDED()) {
                Result.Companion companion = Result.INSTANCE;
                actualCompletion.resumeWith(Result.m170constructorimpl(value));
            }
        } catch (Throwable e) {
            Result.Companion companion2 = Result.INSTANCE;
            actualCompletion.resumeWith(Result.m170constructorimpl(ResultKt.createFailure(e)));
        }
    }
    ...
    public static final <T, R> Object startUndispatchedOrReturn(ScopeCoroutine<? super T> scopeCoroutine, R r, Function2<? super R, ? super Continuation<? super T>, ? extends Object> function2) {
        ...
        Object state$iv;
        try {
            result$iv = ((Function2) TypeIntrinsics.beforeCheckcastToFunctionOfArity(function2, 2)).invoke(r, scopeCoroutine);
        } catch (Throwable e$iv) {
            result$iv = new CompletedExceptionally(e$iv, false, 2, null);
        }
        if (result$iv != IntrinsicsKt.getCOROUTINE_SUSPENDED() && (state$iv = scopeCoroutine.makeCompletingOnce$kotlinx_coroutines_core(result$iv)) != JobSupportKt.COMPLETING_WAITING_CHILDREN) {
            if (state$iv instanceof CompletedExceptionally) {
                ...
                if (DebugKt.getRECOVER_STACK_TRACES() && (continuation instanceof CoroutineStackFrame)) {
                    throw StackTraceRecoveryKt.recoverFromStackFrame(exception$iv$iv, (CoroutineStackFrame) continuation);
                }
            ...
            if (state$iv instanceof CompletedExceptionally) {
                ...
                if (result$iv instanceof CompletedExceptionally) {
                    ...
                    if (DebugKt.getRECOVER_STACK_TRACES() && (continuation2 instanceof CoroutineStackFrame)) {
                        throw StackTraceRecoveryKt.recoverFromStackFrame(exception$iv$iv2, (CoroutineStackFrame) continuation2);
                    }
